...
redisDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "kafka:19092").option("subscribe","redis-server").option("startingOffsets","earliest").load()
...
redisDF.withColumn("value", from_json("value", redisMessageSchema)).select(col('value.existType'), col('value.Ch'), col('value.Incr'), col('value.zSetEntries')).createOrReplaceTempView("RedisSortedSet")
