...
rawDf = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "kafka:19092").option("subscribe", "redis-server").option("startingOffsets", "earliest").load()

# TO-DO: cast the value column in the streaming dataframe as a STRING 
casted_redis_df = rawDf.selectExpr("cast (key as string) key", "cast (value as string) value")
...
RedisSortedSet = casted_redis_df.withColumn("value",from_json("value",redisMessageSchema))\
...
b64_df = spark.sql( "SELECT zSetEntries[0].element AS encodedCustomer FROM RedisSortedSet")
...
CustomerWithContactInfoSchema = StructType(
[
...
clear_df = b64_df.withColumn("customer", unbase64(b64_df.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
json_df = clear_df.withColumn("customer", from_json("customer", CustomerWithContactInfoSchema))\
                             .select(col('customer.*'))\
...
splitted_emailAndBirthDayStreamingDF = emailAndBirthDayStreamingDF.withColumn('birthYear', split(emailAndBirthDayStreamingDF.birthDay,"-").getItem(0)) 
# TO-DO: Select only the birth year and email fields as a new streaming data frame called emailAndBirthYearStreamingDF
emailAndBirthYearStreamingDF = splitted_emailAndBirthDayStreamingDF.select(col('email'), col('birthYear'))

# TO-DO: using the spark application object, read a streaming dataframe from the Kafka topic stedi-events as the source
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
rawDf = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "kafka:19092").option("subscribe", "stedi-events").option("startingOffsets", "earliest").load()

# TO-DO: cast the value column in the streaming dataframe as a STRING 
df = rawDf.selectExpr("cast (key as string) key", "cast (value as string) value")
...
customer_risk_view = df.withColumn("value",from_json("value",stediEventSchema))\
        .select(col('value.*')) \
...
# sink to topic
q1 = joinedDF.selectExpr("cast(customer as string) as key", "TO_JSON(struct(*)) AS value").writeStream.outputMode('append').format("kafka").option("kafka.bootstrap.servers", "kafka:19092").option("FailOnDataLoss" , "false").option("checkpointLocation", "/tmp/my_cp").option("topic", "risk_topic").start();

q2 = joinedDF.selectExpr("TO_JSON(struct(*)) AS value").writeStream.outputMode('append').format('console').option('truncate' , False).start()
